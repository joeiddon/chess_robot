...
state_t state = {{{ ROOK,  KNIGHT,  BISHOP,  QUEEN,  KING,  BISHOP,  KNIGHT,  ROOK},
                  ...
                  {    0,       0,       0,      0,     0,       0,       0,     0},
                  {-PAWN,   -PAWN,   -PAWN,  -PAWN, -PAWN,   -PAWN,   -PAWN, -PAWN},
                  {-ROOK, -KNIGHT, -BISHOP, -QUEEN, -KING, -BISHOP, -KNIGHT, -ROOK}}, 0, 0};
...
void main(){
    ...
    move_t move = {{1,4},{3,4},0,0}; //white king pawn forward by 2
    //move_t move = {{0,4},{0,6},0,1,1}; //castling test, queen kingside
    //move_t move = {{0,0},{1,0},0,0,1}; //castle invalid test, moving rook away
    print_move(&move);
